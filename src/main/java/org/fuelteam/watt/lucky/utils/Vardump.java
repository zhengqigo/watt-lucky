...
class Basic {
    ...
    final protected static Class<?>[] C = { Byte.class, Short.class, Integer.class, Long.class, Float.class,
            Double.class, Boolean.class, Character.class, String.class, StringBuilder.class, StringBuffer.class,
            BigDecimal.class, BigInteger.class };

    final protected static String[] N = { "(byte)", "(short)", "(int)", "(long)", "(float)", "(double)", "(boolean)",
            "(char)", "(string)", "(stringbuilder)", "(stringbuffer)", "(bigdecimal)", "(biginteger)" };
    ...
    protected static <T> void print(String before, T t, String after) {
        if (t == null) append(before + "null" + after);
        boolean b = true;
        for (int i = 0; i < Basic.C.length; i++) {
            Class<?> c = Basic.C[i];
            if (t.getClass().equals(c)) {
    ...
    protected static void append(String str) {
        if (!str.equals("")) content.append(stackToString() + str + "\n");
    }
...
public class Vardump extends Coll {
    ...
    public static void print(Cookie[] cookies) {
        ...
        for (Cookie cookie : cookies) {
            print("{(string)\"" + cookie.getName() + "\"(length=" + cookie.getName().length() + ")=>(string)"
                    + cookie.getValue() + "(length=" + cookie.getValue().length() + ")}");
        }
        printAfter();
        show();
    }
