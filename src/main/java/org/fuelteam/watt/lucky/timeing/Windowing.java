...
public class Windowing {
     ...
     */
    public static <T> Iterable<TimeWindow<T>> toSessionWindows(List<T> source, TimestampSelector<T> selector, long start,
            long end, long timeout) {
        if (source.size() <= 0) {
     ...
     */
    public static <T> Iterable<TimeWindow<T>> toTumblingWindows(List<T> source, TimestampSelector<T> selector, long start,
            long end, long duration) {
     ...
     */
    public static <T> Iterable<TimeWindow<T>> toSlidingWindows(List<T> source, TimestampSelector<T> selector, long start,
            long end, long duration, long every) {
        if (source.size() <= 0) {
            throw new NullPointerException("Underlying source collection has no items.");
        }
    ...
    static class SlidingWindowIterable<T> implements Iterable<TimeWindow<T>> {
        ...
        public SlidingWindowIterable(List<T> source, TimestampSelector<T> selector, long start, long end, long duration,
                long every) {
            _source = source;
            _selector = selector;
            _start = start;
